...
| `/spaces/[slug]` | Venue detail — booking is functional against the local-prototype `bookings` store; requests start `pending` until the owner accepts from `/dashboard/venue` (no quote step). Submitting is review-then-send: "Send booking request" opens a summary dialog, only "Confirm & send" writes the booking, and a success dialog follows with "Go see event details" / "Continue browsing". The booking panel is hidden for the venue's own owner and for anyone whose active role is venue/vendor/admin (UX only — the real gate is Phase 2 server-side). `?from=dashboard` sends the back link to `/dashboard` instead of Discover Spaces. `?date=&start=&end=&attendees=&eventType=` (the same keys Discover Spaces filters on) pre-fill the booking form — see `src/lib/spaces/bookingPrefill.ts`; every value is still editable and still re-validated by `evaluateBookingRequest()` |
...
| `/vendors` | Planner-facing vendor search — real (prototype), lists `published` vendor profiles. Reads a `VendorSearchBrief` from the query string (`category`/`date`/`location`/`guests`/`budget`/`needs`, see `src/lib/vendors/vendorSearch.ts`), on top of the free-text/remote filters. Reachable without a booking — this is the "I already have a venue" path |
...
| `/dashboard/organizer/bookings/[bookingId]/vendors` | organizer who owns the booking | Real (prototype) — post vendor requests, review the roster. `?post=1` opens the post-a-request form on arrival (read server-side; linked from the terms-accepted dialog) |
