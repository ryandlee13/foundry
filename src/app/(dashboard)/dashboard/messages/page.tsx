...
export default function MessagesInboxPage() {
  ...
  const refresh = useCallback(() => {
    ...
    const byBooking = new Map<string, EventGroup>();
    ...
    for (const thread of threads) {
      ...
      const bookingId = need?.bookingId ?? thread.id;
      const booking = need ? getBookingById(need.bookingId) : undefined;
      const eventLabel = booking ? formatEventLabel(booking) : "Vendor requests";
      const activityAt = summary.lastMessageAt ?? thread.createdAt;

      const existing = byBooking.get(bookingId);
      if (existing) {
        existing.threads.push(summary);
        if (activityAt > existing.mostRecentAt) existing.mostRecentAt = activityAt;
      } else {
        byBooking.set(bookingId, { bookingId, eventLabel, threads: [summary], mostRecentAt: activityAt });
      }
    }

    const groupList = Array.from(byBooking.values());
  ...
  return (
    <div>
      ...
      <p className="mt-1 text-sm text-ink-soft">Grouped by event — start a conversation with a vendor from their proposal.</p>

      <div className="mt-8 space-y-6">
        {groups.length === 0 ? (
          <EmptyState title="No conversations yet" description="Start a conversation with a vendor from their proposal to begin messaging." />
        ) : (
          groups.map((group) => (
